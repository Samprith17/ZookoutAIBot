"""
Milestone 13.1 - Merchant Intent Router
Dedicated router that intercepts all merchant growth, offer review, dashboard, health, and promotion commands.
...
MERCHANT_ROUTES = {
    ...
    "merchant_promote": ["which offer should i promote?", "which offer should i promote", "what deal to promote", "/promote"],
...
def route_merchant_intent(message: str) -> Optional[Dict[str, Any]]:
    """
    Evaluates message against Merchant Routes.
    ...
    for intent_type, triggers in MERCHANT_ROUTES.items():
        if any(tr == text or text.startswith(tr) or text.endswith(tr) for tr in triggers):
            logger.info(f"[Merchant Router Intercepted]: {intent_type} for message: '{message}'")
            return {
                "type": intent_type,
                "is_merchant": True,
                "query": message
